...
void sortProcessesForExecution(int n, double burst_time[], int priority[], int processOrder[]) {
    for(int i{1}; i<=n; i++) {
        for(int j{1}; j<=n-i; j++) {
...
void nonPreemptivePriority(int n, double burst_time[], int priority[]) {
    ...
    sortProcessesForExecution(n, burst_time, priority, processOrder);
...
int main() {
    ...
    std::cin >> n;
    std::cout << "Enter the burst time and priorities for each process : " << std::endl;
    for(int i{1}; i<=n; i++) {
    ...
    nonPreemptivePriority(n, burst_time, priority);
    return 0;
}